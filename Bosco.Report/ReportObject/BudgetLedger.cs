...
namespace Bosco.Report.ReportObject
{
    public partial class BudgetLedger : Bosco.Report.Base.ReportHeaderBase
    {
            ...
            DateTime dtPDateFrom, DateTime dtPDateTo, Int32 budgetAction, int PreviousBudgetId)
        {
            ...
            if (LoginUser.IS_ABEBEN_DIOCESE || LoginUser.IS_DIOMYS_DIOCESE || LoginUser.IS_CMF_CONGREGATION || LoginUser.IS_BSG_CONGREGATION)
            {
            ...
            using (DataManager dataManager = new DataManager(SQL.ReportSQLCommand.BudgetVariance.BudgetLedgers, DataBaseType.HeadOffice))
            {
            ...
            if (resultArgs.Success && resultArgs.DataSource.Table != null)
            {
                ...
                dtIncomeSource.DefaultView.RowFilter = "PROPOSED_CURRENT_YR > 0 OR APPROVED_CURRENT_YR > 0";
                ...
                else if (LoginUser.IS_CMF_CONGREGATION && transmode == TransMode.DR)
                {
                    ...
                    xrGSumApproved.Text = UtilityMember.NumberSet.ToNumber(totalApprovedBudgetedExpenseordinary + totalApprovedBudgetedExpenseextraordinary + totalApprovedBudgetedExpensewithoutboth);
                }
                else
                {
                    totalPrevApprovedBudgetedExpense = UtilityMember.NumberSet.ToDouble(dtIncomeSource.Compute("SUM(APPROVED_PREVIOUS_YR)", string.Empty).ToString());
                ...
                //07/07/2020
                if (LoginUser.IS_CMF_CONGREGATION || LoginUser.IS_BSG_CONGREGATION)
                {
        ...
        private void AppendCashBankBalances(DataTable dtIncomeBudget)
        {
            ...
            double CashOPBalance = this.GetBalance(BudgetProjects, UtilityMember.DateSet.ToDate(dtBudgetDateFrom.ToShortDateString()), BalanceSystem.LiquidBalanceGroup.CashBalance,
            ...
            double BankOPBalance = this.GetBalance(BudgetProjects, dtBudgetDateFrom.ToShortDateString(), BalanceSystem.LiquidBalanceGroup.BankBalance,
                     BalanceSystem.BalanceType.OpeningBalance);
        ...
        {
            if (this.LoginUser.IS_CMF_CONGREGATION || this.LoginUser.IS_BSG_CONGREGATION)
            {
        ...
        private void xrgrpBudgetGroupSubTotal_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            {
                if (
                    (LoginUser.IS_CMF_CONGREGATION || LoginUser.IS_BSG_CONGREGATION)
                    && GetCurrentColumnValue(reportSetting1.BUDGETVARIANCE.BUDGET_GROUPColumn.ColumnName) != null
                )
                {
