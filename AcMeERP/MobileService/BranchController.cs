...
namespace AcMeERP.MobileService
{
    public class BranchController : ApiController
    {
        public List<ServiceResult> Post(HttpRequestMessage request)
        {
            ...
            BranchData dtData = JsonConvert.DeserializeObject<BranchData>(array[0].ToString());
            string errorMessage = ValidateBranchData(dtData);
            if (string.IsNullOrEmpty(errorMessage))
            {
                switch (dtData.MobileRequest)
                {
                    case "branch":
                        try
                        {
                            new ErrorLog().WriteMobileServiceError("Reached Branch Controller");
                            using (MobileServiceSystem mobileServiceSystem = new MobileServiceSystem())
                            {
                                ...
                                if (resultArgs != null && resultArgs.Success)
                                {
                                    ...
                                    if (resultArgs != null && resultArgs.Success && resultArgs.RowsAffected > 0)
                                    {
                                        ...
                                        {
                                            resultArgs = mobileServiceSystem.GetAllBranchesByHeadoffice();
                                        }
                                        else
                                        {
                                    ...
                                    new ErrorLog().WriteMobileServiceError("Branch Controller Service Request Completed");
                                }
                                if (resultArgs.Success)
                                {
                                    ...
                                    objResult.Data.Add("BranchDetails", resultArgs.DataSource.Table);
                                }
                                else
                                {
        ...
        private string ValidateBranchData(BranchData dtData)
        {
