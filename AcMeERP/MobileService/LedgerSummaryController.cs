...
{
    public class LedgerSummaryController : ApiController
    {
        ...
        public List<ServiceResult> Post(HttpRequestMessage request)
        {
            ...
            LedgerSummaryData dtData = JsonConvert.DeserializeObject<LedgerSummaryData>(array[0].ToString());
            string errorMessage = ValidateLedgerSummaryData(dtData);
            if (string.IsNullOrEmpty(errorMessage))
            {
                switch (dtData.MobileRequest)
                {
                    case "ledgersummary":
                        try
                        {
                            new ErrorLog().WriteMobileServiceError("Reached Ledger Summary Controller");
                            using (MobileServiceSystem mobileServiceSystem = new MobileServiceSystem())
                            {
                                ...
                                if (resultArgs != null && resultArgs.Success)
                                {
                                    ...
                                    if (resultArgs != null && resultArgs.Success)
                                    {
                                        ...
                                        {
                                            //mobileServiceSystem.LedgerGroup = dtData.LedgerGroup;
                                            mobileServiceSystem.LedgerName = dtData.LedgerName;
                                            ...
                                            resultArgs = mobileServiceSystem.GetLedgerSummary();
                                        }
                                        else
                                        {
                                ...
                                new ErrorLog().WriteMobileServiceError("LedgerSummary Controller Service Request Completed");
                            }
                        }
                        catch (Exception ex)
                        {
                            objResult.Status = MessageCatalog.MobileService.FalseStatus; ;
                            objResult.ErrorCode = MessageCatalog.MobileService.InvalidLoginErrorCode;
                            objResult.ErrorMessage = "Error in Getting Ledger Summary Details";
        ...
        private string ValidateLedgerSummaryData(LedgerSummaryData dtData)
        {
            ...
            else if (string.IsNullOrEmpty(dtData.LedgerName))
            {
                Message = MessageCatalog.MobileService.LedgerNameEmpty;
            }
            return Message;
        }
    }
