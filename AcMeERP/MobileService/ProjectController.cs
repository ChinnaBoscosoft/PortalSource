...
namespace AcMeERP.MobileService
{
    public class ProjectController : ApiController
    {
        public List<ServiceResult> Post(HttpRequestMessage request)
        {
            ...
            ProjectData dtData = JsonConvert.DeserializeObject<ProjectData>(array[0].ToString());
            string errorMessage = ValidateProjectData(dtData);
            if (string.IsNullOrEmpty(errorMessage))
            {
                switch (dtData.MobileRequest)
                {
                    case "project":
                        try
                        {
                            new ErrorLog().WriteMobileServiceError("Reached Project Controller");
                            using (MobileServiceSystem mobileServiceSystem = new MobileServiceSystem())
                            {
                                ...
                                if (resultArgs != null && resultArgs.Success)
                                {
                                    ...
                                    if (resultArgs != null && resultArgs.Success)
                                    {
                                        ...
                                        if (dtUser.Rows.Count > 0)
                                        {
                                            ...
                                            resultArgs = mobileServiceSystem.GetProjectsByBranch();
                                        }
                                        else
                                        {
                                ...
                                new ErrorLog().WriteMobileServiceError("Project Controller Service Request Completed");
                            }
                        }
                        catch (Exception ex)
                        {
                            objResult.Status = MessageCatalog.MobileService.FalseStatus; ;
                            objResult.ErrorCode = MessageCatalog.MobileService.InvalidLoginErrorCode;
                            objResult.ErrorMessage = "Error in Getting Project Details";
        ...
        private string ValidateProjectData(ProjectData dtData)
        {
